...
def load(name):
    """
        Load the backbone model specified by name.

        Parameters:
        - name (str): The name of the backbone model to load.

        Returns:
        - backbone: The loaded model or None if the model name is invalid.
        """
    return eval(_BACKBONES[name])


if __name__ == '__main__':
    net = load('wideresnet50')
    print(net)
    import torch
    from torchsummary import summary
    mod = models.wide_resnet50_2(pretrained=True).cuda()
    new_net = torch.nn.Sequential(*list(mod.layer2.children())
                                  + list(mod.layer3.children())
                                  + list(mod.layer4.children()))
    print(new_net)
    summary(mod, (3, 288, 288), batch_size=8)
    print(mod)

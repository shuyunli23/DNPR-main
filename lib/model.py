...
from lib.utils import os, torch, plot_segmentation_images_for_paper, normalize_data, plot_segmentation_images, \
    compute_metrics, batch_center_crop, feature_select, select_reference_image_with_orb, np, select_features
from lib.common import F, PatchMaker, NetworkFeatureAggregator, Preprocessing, Aggregator, RescaleSegment, \
    FeatureMapProjection, AffineTrans


class DyNorm:
    """
    DyNorm: zero-shot industrial anomaly detection via dynamic normal prototype evolution.
    """
    ...
    def __init__(
        ...
        self.reg_msk = None
        self.edge_msk = self.get_edge_mask(input_shape)
    ...
    def get_edge_mask(self, feat_resolution, retention_rate=0.94):
    ...
    def feature_update(self, _features, instance_scores, patch_scores):
        ...
        if num == 0:
            if _num <= self.glo_memory_num:
                ...
                scores, indices = torch.topk(instance_scores, self.glo_memory_num + memory_limit, dim=0, largest=False)
                self.glo_scores = scores[:self.glo_memory_num]
                self.glo_memory = _features[indices[:self.glo_memory_num]]
                self.gol_patch_scores = patch_scores[indices[:self.glo_memory_num]]
                self._feature_update(
                    _features[indices[self.glo_memory_num:]], patch_scores[indices[self.glo_memory_num:]]
                )
        else:
            if (num + _num) <= self.glo_memory_num:
                self.glo_memory = torch.cat([self.glo_memory, _features], dim=0)
                ...
                cat_features = torch.cat([self.glo_memory, _features], dim=0)
                scores, indices = torch.topk(cat_scores, self.glo_memory_num + memory_limit, dim=0, largest=False)
                self.glo_scores = scores[:self.glo_memory_num]
                ...
                self._feature_update(
                    cat_features[indices[self.glo_memory_num:]], cat_patch_scores[indices[self.glo_memory_num:]]
                )

    def _feature_update(self, _features, _scores):
        num = 0 if self.loc_memory is None else self.loc_memory.size(0)
        _num = _features.size(0)
        if num == 0:
            if _num <= self.loc_memory_num:
                self.loc_memory = _features
                self.loc_scores = _scores
            else:
                # self.loc_memory, self.loc_scores = feature_select(_features, _scores, self.loc_memory_num)
                self.loc_memory, self.loc_scores = select_features(_features, _scores, self.loc_memory_num)
        else:
            if (num + _num) <= self.loc_memory_num:
                self.loc_memory = torch.cat([self.loc_memory, _features], dim=0)
                self.loc_scores = torch.cat([self.loc_scores, _scores], dim=0)
            else:
                # self.loc_memory, self.loc_scores = feature_select(
                #     torch.cat([self.loc_memory, _features], dim=0), torch.cat([self.loc_scores, _scores], dim=0),
                #     self.loc_memory_num
                # )
                self.loc_memory, self.loc_scores = select_features(
                    torch.cat([self.loc_memory, _features], dim=0), torch.cat([self.loc_scores, _scores], dim=0),
                    self.loc_memory_num
                )
    ...
    def batch_predict(self, input_images, matrix, reg_model):
        feature_maps = self.features_extract(input_images)
        feature_maps = self.dim_reduction(feature_maps)
        b, c, h, w = feature_maps.shape
        ...
        if matrix is not None:
            ...
            revised_patch_scores = reg_model.affine_trans(matrix_inv, patch_scores.unsqueeze(1),
                                                          padding_mode='zeros', need_blank_mask=False).squeeze(1)
            revised_patch_scores = revised_patch_scores * self.re_size(self.reg_msk, (h, w)).squeeze(1)
        else:
            revised_patch_scores = patch_scores * self.re_size(self.edge_msk, (h, w)).squeeze(1)
    ...
    def predict(self):
        ...
        end_time = time.time()
        # np.save(f'{self.args.save_path}/loc_memory.npy', self.loc_memory.cpu().numpy())
...
if __name__ == '__main__':
    ...
    def test_one():
        ...
        # choose = "visa"
        # choose = "ci"
        print(conf.glo_memory_num)
        print(conf.loc_memory_num)
        ...
        if choose == 'mvtec':
            ...
            # choose_strategy = {
            #     'pcb1': 'transform',
            #     **{name: 'remain' for name in VISA_CLASS_NAMES if
            #        name not in ['pcb1']}
            # }
            # choose_strategy = {
            #     'pcb1': 'transform',
